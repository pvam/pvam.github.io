...
filename  = '../dataop/HashJoin.train'
...
for line in ip:
	...
	nl = np.float(elm[0])
	nr = np.float(elm[1])
	A.append([nl * math.log(nr) + nl])
	# A.append([nl])
	# A.append([nr * math.log(nl,2)])
	# A.append([nl * math.log(nr,2)])
...
plt.xlabel('Nr* log2(Nl)')
plt.ylabel('Execution time(ms) ')
plt.title(operator + '- Linear regression, Explained variance: ' + exp_variance + ', error : ' + error)
plt.legend()
plt.show()