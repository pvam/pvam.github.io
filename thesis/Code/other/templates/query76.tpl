...
[_LIMITA] select [_LIMITB] channel, col_name, d_year, d_qoy, i_category, COUNT(*) as sales_cnt, SUM(ext_sales_price) as sales_amt FROM (
        SELECT 'store' as channel, '[NULLCOLSS]' as col_name, d_year, d_qoy, i_category, ss_ext_sales_price as ext_sales_price
         FROM store_sales, item, date_dim
         WHERE [NULLCOLSS] IS NULL
           AND ss_sold_date_sk=d_date_sk
           AND ss_item_sk=i_item_sk
        UNION ALL
        SELECT 'web' as channel, '[NULLCOLWS]' as col_name, d_year, d_qoy, i_category, ws_ext_sales_price as ext_sales_price
         FROM web_sales, item, date_dim
         WHERE [NULLCOLWS] IS NULL
           AND ws_sold_date_sk=d_date_sk
           AND ws_item_sk=i_item_sk
        UNION ALL
        SELECT 'catalog' as channel, '[NULLCOLCS]' as col_name, d_year, d_qoy, i_category, cs_ext_sales_price as ext_sales_price
